        ...
        public sealed class sedawntime : mimaextensions.mimaexpartner
        {
            ...
            private IEnumerable<BattleAction> OnOwnerTurnStarted(UnitEventArgs args)
            {
                if (ThisTurnActivating)
                {
                    ...
                    List<Card> list = toolbox.RollCardsCustomIgnore(GameRun.BattleCardRng, new CardWeightTable(RarityWeightTable.BattleCard, OwnerWeightTable.OnlyPlayer, CardTypeWeightTable.CanBeLoot), Level, null, false, false, false, false, (Card card) => !card.Keywords.HasFlag(Keyword.Forbidden) && card.Config.Id != "carddawntime").ToList();
                    if (list.Count > 0)
                    {
