...
{
	public sealed class sepotehhbrDef : lvalonmimaStatusEffectTemplate
	{
	...
	[EntityLogic(typeof(sepotehhbrDef))]
	public sealed class sepotehhbr : StatusEffect
	{
		public int Value1 => Owner == null ? 1 : Level;
		public ManaGroup Mana => new ManaGroup() { Hybrid = 2, HybridColor = 7 };
		...
		private IEnumerable<BattleAction> OnManaConsumed(ManaEventArgs args)
		{
			if (Battle.AllAliveEnemies.Count() > 0)
			{
				...
				{
					NotifyActivating();
					yield return BuffAction<Charging>(item * Value1, 0, 0);
				}
			}
		}
	}
